...
mass = {23: 91., 25: 125., 443: 3.096, 553: 9.460}
...
class ResonanceBuilder(Analyzer):
    ...
    def process(self, event):
        ...
        resonances.sort(key=lambda x: abs(x.mass()-nominal_mass)) # sort the dimuons by their distance to the pdg jpsi mass
        event.dimuons = [ res for res in resonances if self.cfg_ana.filter_func(leg) ] # fill the dimuon category if leg cut is true.///doubtful
        event.dimuons = event.dimuons[:self.cfg_ana.ndimuons]
#        resonances = [res for res in resonances if self.cfg_ana.filter_func2(resonances)] #fill the dimuon category if dimu mass is inbounds
